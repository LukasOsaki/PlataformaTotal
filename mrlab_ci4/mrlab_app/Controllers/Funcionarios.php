...
class Funcionarios extends PainelController
{
	protected $funcMD = null;
	protected $arqMD = null;
	protected $funcDepMD = null;
	protected $cfg = null;
	...
	public function __construct()
	{
		...
		$this->funcDepMD = new \App\Models\FuncionariosDependentesModel();
	...
	public function form($func_id = 0)
	{
		...
		if ($query && $query->resultID->num_rows >= 1) {
			...
			//Dependentes
			$query_dependentes = $this->funcDepMD
				->where('func_id', $func_id)
				->where('func_dep_tipo', 'Curso') // Filtro adicional
				->orderBy('func_dep_id', 'ASC')
				->get();
			if ($query_dependentes && $query_dependentes->resultID->num_rows >= 1) {
				$this->data['rs_list_dependentes'] = $query_dependentes;
			}
		}
	...
	public function update_dependentes($args = [])
	{
		$file_name = (isset($args['file_name']) ? $args['file_name'] : '');
		$file_prefixo = (isset($args['file_prefixo']) ? $args['file_prefixo'] : '');
		$func_id = (int)(isset($args['func_id']) ? $args['func_id'] : '');
	...
	public function ajaxform($action = "")
	{
		...
		switch ($action) {
			case "EXCLUIR-ARQUIVO":
				...
				exit();
				break;
			case "EXCLUIR-DEPENDENTE":
				$func_id = (int)$this->request->getPost('func_id');
				$dep_hashkey = $this->request->getPost('func_dep_hashkey');
				if (!empty($dep_hashkey)) {
					$query = $this->funcDepMD
						->where('func_id', $func_id)
						->where('func_dep_hashkey', $dep_hashkey)
						->limit(1)
						->get();
					if ($query && $query->resultID->num_rows >= 1) {
						$rs_dados = $query->getRow();
						$this->funcDepMD->where('func_id', $func_id);
						$this->funcDepMD->where('func_dep_hashkey', $dep_hashkey);
						$this->funcDepMD->delete();
