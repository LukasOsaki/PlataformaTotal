...
class Pendencias extends PainelController
{
	...
	public function form( $pend_id = 0 )
	{
		if ($this->request->getPost())
		{
			...
			if ($this->validate($rules)) {
				...
				//exit();
				if( is_array($arr_pendtag_tag)){
					foreach ($arr_pendtag_tag as $key => $val) {
						//echo('<hr>');
						$pendtag_tag = $val;
						...
						print '<div>
							<div>'. $val .'</div>
							<div>'. $pendtag_descricao .'</div>
							<div>'. $pendtag_observacoes .'</div>
						</div>';

						if( !empty($pendtag_tag) )
						{
							$acaoTAGS = 'INSERT';
							if( $pendtag_id > 0 ){
							...
							//$tvlr_urlpage = 'tarf_id-'. $tarf_id .'-'. url_title( convert_accented_characters($tvlr_titulo), '-', TRUE ); 
							$data_tag_db = [
								'pend_id' => $pend_id, 
