								...
								<div class="row align-items-center">
									...
									<div class="col-12 col-md-6">
										...
										<div class="d-flex justify-content-end">
											<div style="margin-left: 5px;"><a href="<?php echo(site_url('pendencias/form')); ?>" class="btn btn-sm btn-primary">Novo Registro</a></div>
										</div>
												...
												<table id="example2" class="display nowrap table table-striped table-bordered" style="width:100%">
													<thead>
														<tr>
															...
															<th style="width:90px;">Status/Tag</th>
															<th>Descrição</th>
													...
													<?php
														...
														foreach ($rs_list->getResult() as $row) {
															$count++;
															$pend_id = ($row->pend_id);
															$pend_hashkey = ($row->pend_hashkey);
															$clie_nome_razao = ($row->clie_nome_razao);
															$pend_dte_registro = fct_formatdate($row->pend_dte_registro, 'd/m/Y');
															$pend_status = ($row->categ_titulo);
															$categ_color = ($row->categ_color);
															//$label_status = (isset($cfgCategAreas[$pend_status]['label']) ? $cfgCategAreas[$pend_status]['label'] : '');

															$pend_num_os = ($row->pend_num_os);
															$pend_tag = ($row->pend_tag);
															$pend_descricao = ($row->pend_descricao); 
															$pend_dte_instalacao = fct_formatdate($row->pend_dte_instalacao, 'd/m/Y');

															$link_form = site_url('pendencias/form/'. $pend_id);
															...
															<tr class="trRow">
																<td class="text-center">
																	<div class="d-flex justify-content-center">
																		...
																		<div style="margin: 0 3px;">
																			<a href="javascript:;" class="btn btn-sm btn-ac btn-danger cmdExcluirRegistro" data-hashkey ="<?php echo($pend_hashkey); ?>"><i class="las la-trash"></i></a>
																		</div>
																	</div>
																</td>
																<td><?php echo($pend_id); ?></td>
																<td><?php echo($clie_nome_razao); ?></td>
																<td><?php echo($pend_dte_registro); ?></td>
																<td style="background-color: <?php echo($categ_color); ?>">
																	<div><strong><?php echo($pend_status); ?></strong></div>
																	<div><?php echo($pend_tag); ?></div>
																</td>
																<td><?php echo($pend_descricao); ?></td>
																<td><?php echo($pend_dte_instalacao); ?></td>
															</tr>
														<?php
														}
													?>
