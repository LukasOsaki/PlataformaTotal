								...
								<div class="row align-items-center">
									...
									<div class="col-12 col-md-6">
										...
										<div class="d-flex justify-content-end">
											<div style="margin-left: 5px;"><a href="<?php echo(site_url('equipamentos/form')); ?>" class="btn btn-sm btn-primary">Novo Registro</a></div>
										</div>
												...
												<table id="example2" class="display table table-striped table-bordered" style="width:100%">
													...
													<?php
														...
														foreach ($rs_list->getResult() as $row) {
															$count++;
															$eqto_id = ($row->eqto_id);
															$eqto_hashkey = ($row->eqto_hashkey);
															$eqto_titulo = ($row->eqto_titulo);
															$eqto_tag = ($row->eqto_tag);
															$eqto_setor = ($row->eqto_setor);
															$clie_nome_razao = ($row->clie_nome_razao);

															$link_form = site_url('equipamentos/form/'. $eqto_id);
															...
															<tr class="trRow">
																<td class="text-center">
																	<div class="d-flex justify-content-center">
																		...
																		<div style="margin: 0 3px;">
																			<a href="javascript:;" class="btn btn-sm btn-ac btn-danger cmdExcluirRegistro" data-hashkey ="<?php echo($eqto_hashkey); ?>"><i class="las la-trash"></i></a>
																		</div>
																	</div>
																</td>
																<td><?php echo($eqto_id); ?></td>
																<td><?php echo($eqto_titulo); ?></td>
																<td><?php echo($eqto_tag); ?></td>
																<td><?php echo($eqto_setor); ?></td>
			...
			$(document).on('click', '.cmdExcluirRegistro', function (e) {
				...
				Swal.fire({
					...
					if (result.value) {
						// ------------------------------------------------------
						let $formData = {
							eqto_hashkey: $hashkey
						};

						$.ajax({
							url: site_url  +'equipamentos/ajaxform/EXCLUIR-EQUIPAMENTO',
							method:"POST",
							type: "POST",
							dataType: "json",
							data: $formData,
