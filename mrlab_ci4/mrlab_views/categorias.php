								...
								<div class="row align-items-center">
									...
									<div class="col-12 col-md-6">
										...
										<div class="d-flex justify-content-end">
											<div style="margin-left: 5px;"><a href="<?php echo(site_url('categorias/form')); ?>" class="btn btn-sm btn-primary">Novo Registro</a></div>
										</div>
												...
												<table id="example2" class="display nowrap table table-striped table-bordered" style="width:100%">
													...
													<?php
														...
														foreach ($rs_list->getResult() as $row) {
															...
															$categ_hashkey = ($row->categ_hashkey);
															$categ_titulo = ($row->categ_titulo);
															$categ_area = ($row->categ_area);

															$label_area = (isset($cfgCategAreas[$categ_area]['label']) ? $cfgCategAreas[$categ_area]['label'] : "");


															$categ_color = ($row->categ_color);


															$link_form = site_url('categorias/form/'. $categ_id);
															$linkGerarPDF = site_url();
														?>
															<tr class="trRow" style="background-color: <?php echo($categ_color); ?> ">
																<td class="text-center">
																	<div class="d-flex justify-content-center">
																		...
																		<div style="margin: 0 3px;">
																			<a href="javascript:;" class="btn btn-sm btn-ac btn-danger cmdExcluirRegistro" data-hashkey ="<?php echo($categ_hashkey); ?>"><i class="las la-trash"></i></a>
																		</div>
																	</div>
																</td>
																<td><?php echo($categ_id); ?></td>
																<td><?php echo($categ_titulo); ?></td>
																<td><?php echo($label_area); ?></td>
																<td><?php echo($categ_color); ?></td>
															</tr>
														<?php
														}
													?>
			...
			$(document).on('click', '.cmdExcluirRegistro', function (e) {
				...
				Swal.fire({
					...
					if (result.value) {
						// ------------------------------------------------------
						let $formData = {
							categ_hashkey: $hashkey
						};

						$.ajax({
							url: site_url  +'categorias/ajaxform/EXCLUIR-REGISTRO',
							method:"POST",
							type: "POST",
							dataType: "json",
							data: $formData,
